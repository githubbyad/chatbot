...
function loadTable(page = 1) {
    fetch("load-responses.php?page=" + page)
        ...
        .then(data => {
            ...
            for (var i in data) {
                output += `<tr>
                <td class="fw-bold">${data[i].category}</td>
...
function pagination(){
    ...
    .then(data => {
        ...
        for(var i = 1; i < data.pages; i++) {
            j = i + 1;
            output += `<li class="page-item" data-page="${j}" onclick="loadTable(${j})"><a class="page-link" href="javascript:void(0)">${j}</a></li>`;
        }
        output += `</ul>`;
...
document.querySelector("#insert_data").addEventListener("click", e => {
    e.preventDefault();
    // get form values
    var f_category = document.querySelector("#category").value;
...
document.querySelector("#update_data").addEventListener("click", e => {
    ...
    var f_category = document.querySelector("#edit_category").value;
...
function edit_form(id) {
    fetch("get-response.php?id=" + id)
        ...
        .then(data => {
            ...
            document.querySelector("#edit_category").value = data.category;
...
document.querySelector("#searchCategory").addEventListener("keyup", e => {
    document.querySelector("#searchData").value = "";
    var category = document.querySelector("#searchCategory").value;
    if(category != "") {
        document.querySelector(".pages").classList.add('d-none');
        fetch("search-category.php?category=" + category)
            .then(res => res.json())
            .then(data => {
